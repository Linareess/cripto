...
class DB
{
    ...
    public static function in(array|string $keys, array|string $values, array|object $options = []): string
    {
    ...
    public static function like(string $key, array|string $value, array|object $options = []): string
    {
        $array = \RegularLabs\Library\ArrayHelper::applyMethodToValues([$key, $value, $options], '', '', 1);
        if (!is_null($array)) {
            return $array;
        }
     ...
     */
    public static function notIn(string|array $keys, string|array $values, array|object $options = []): string
    {
